...
- Resolve via `GET /api/v1/entities/organizations/by-domain/{domain}` or `search-by-name`.
...
3. If a name search (`search-by-name`) returns a result under a surprising domain (e.g. `thisisironclad.com` for Ironclad), use that domain for the AlphaLens profile link but keep the real favicon domain.
...
- **Bad data matches**: name search can match wrong companies (e.g. Evisort → `pjsherman.net`). Cross-check domain vs. expected company.
...
- **Reindex endpoint**: `POST https://api-venture-production.deckmatch.com/api/v1/indexer/reindex/organization?organization_id={id}` — use the `deckmatch.com` base URL, not `alphalens.ai`.
