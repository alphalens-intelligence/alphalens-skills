...
Read [workflows/pipeline-enrichment.md](workflows/pipeline-enrichment.md) for the full pipeline CRUD workflow.
...
## Mapping Workflow Selection (read this BEFORE doing any work)

When a user asks for a "mapping", "market map", "competitive landscape", or similar, **stop and determine which workflow to run before making any API calls.**

There are two distinct workflows:

| Trigger phrase | Workflow | What it produces |
|---|---|---|
| "market map", "competitive landscape", "who competes with X" | **Org-Level Market Map** | Single-page cluster grid of similar organizations |
| "bottom-up mapping", "bottom-up analysis", "deep dive", "full mapping" | **Bottom-Up Market Intelligence Suite** | Product-centric tabbed market map + investor network + peer benchmark (3 HTML files) |

### Decision rules

1. **If the user explicitly says "bottom-up"** → always use the Bottom-Up Market Intelligence Suite.
2. **If the user explicitly says "market map" or "competitive landscape"** → use the Org-Level Market Map Workflow.
3. **If the request is ambiguous** (e.g. "map out X", "research the space around X", "show me X's competitors") → **ask the user before proceeding**:
   > "I can do this two ways:
   > - **Market map** — a single-page grid showing similar companies clustered by category. Faster, good for a quick overview.
   > - **Bottom-up mapping** — a deeper product-level analysis that produces three linked pages: a product-centric market map (one tab per distinct product line), an investor network graph, and a peer benchmark with headcount/funding charts. More thorough, takes longer.
   >
   > Which would you prefer?"

**Never guess.** The two workflows produce fundamentally different outputs and the bottom-up suite requires significantly more API calls. Getting this wrong wastes the user's time and credits.

---

## Workflow Routing

Once you have determined which workflow to run, read the appropriate files **before proceeding with any API calls**.

### If routed to org-level map:

Read these two files:
1. [workflows/favicon-proxy.md](workflows/favicon-proxy.md) — start the proxy server
2. [workflows/org-market-map.md](workflows/org-market-map.md) — follow Steps 1-6

### If routed to bottom-up suite:

Read [workflows/bottom-up-suite.md](workflows/bottom-up-suite.md) first — it contains the execution order and will direct you to read the following workflows sequentially as you reach each phase:
1. [workflows/favicon-proxy.md](workflows/favicon-proxy.md)
2. [workflows/product-market-map.md](workflows/product-market-map.md)
3. [workflows/investor-network.md](workflows/investor-network.md)
4. [workflows/peer-benchmark.md](workflows/peer-benchmark.md)

### If routed to pipeline enrichment:

Read [workflows/pipeline-enrichment.md](workflows/pipeline-enrichment.md).
